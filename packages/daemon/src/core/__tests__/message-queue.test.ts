...
describe("MessageQueue", () => {
  ...
  it("enqueues and delivers a message to a ready agent", async () => {
    ...
    // The enqueue triggers processQueues internally, advance timers for it
  ...
  it("rate limits delivery after 10 messages per agent in 60 seconds", async () => {
    ...
    // Rate limiting happens during deliver(), so the message is dequeued but not sent
  ...
  it("collapses newlines to ' | ' in terminal mode", async () => {
    ...
    // Check that sendKeys was called with collapsed message
  ...
  it("delays delivery when agent is busy (Thinking)", async () => {
    ...
    // First poll — agent not ready, should not deliver
    await vi.advanceTimersByTimeAsync(100);

    // sendKeys may have been called by the immediate processQueues in enqueue
    // but the delivery should check readiness first
  ...
  it("uses shorter polling interval when messages are queued", async () => {
    ...
    // The first scheduleNextPoll checks queues and picks 500ms
  ...
  it("uses longer polling interval when no messages are queued", async () => {
    ...
    // Check that a 2000ms timeout was scheduled (idle polling)
    const timeouts = setTimeoutSpy.mock.calls.map((call) => call[1]);
  ...
  it("delivers via mcp-pending for registered MCP agents", async () => {
    ...
    // Should write to mcp-pending directory (not inbox)
    expect(fsMock.mkdir).toHaveBeenCalledWith(
  ...
  it("removes agent queues and MCP registration on removeAgent", () => {
    ...
    // Enqueue again — should start fresh (no loop detection carry-over)
